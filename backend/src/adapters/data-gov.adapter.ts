...
export class DataGovAdapter implements GovernmentSourceAdapter {
  readonly sourceId = 'DATA_GOV_IN';
  readonly sourceName = 'Open Government Data Platform India (data.gov.in)';
  readonly baseUrl = 'https://data.gov.in';
  readonly isLiveApiAvailable = true;

  constructor(_apiKey?: string) {
    // Registered API key integration
  }

  private async loadOfficialDataset(): Promise<RawTenderRecord[]> {
    try {
      const datasetPath = join(process.cwd(), '../datasets/data_gov_rural_procurement.json');
      const data = await readFile(datasetPath, 'utf8');
      return JSON.parse(data) as RawTenderRecord[];
    } catch {
      try {
        const fallbackPath = join(process.cwd(), 'datasets/data_gov_rural_procurement.json');
        const data = await readFile(fallbackPath, 'utf8');
        return JSON.parse(data) as RawTenderRecord[];
      } catch (err) {
        console.error('Failed to load data.gov.in dataset from disk:', err);
        return [];
      }
    }
  }
  ...
  async importDataset(rawContent: string, format: 'json' | 'csv'): Promise<RawTenderRecord[]> {
    ...
    throw new Error('CSV parsing for data.gov.in requires standard dataset mapping');
  }
}
