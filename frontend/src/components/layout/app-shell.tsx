...
const ALL_NAV = [
  ...
  { name: 'Tenders', to: '/tenders', icon: DocumentTextIcon, roles: ['CITIZEN', 'OFFICIAL', 'ADMIN', 'AUDITOR'] as UserRole[] },
  { name: 'Vendors', to: '/vendors', icon: BuildingOffice2Icon, roles: ['CITIZEN', 'OFFICIAL', 'ADMIN', 'AUDITOR'] as UserRole[] },
  { name: 'Govt Sources', to: '/sources', icon: BuildingLibraryIcon, roles: ['OFFICIAL', 'ADMIN'] as UserRole[] },
  { name: 'Blockchain Verification', to: '/verification', icon: CircleStackIcon, roles: ['CITIZEN', 'OFFICIAL', 'ADMIN', 'AUDITOR'] as UserRole[] },
...
const ROLE_BADGE: Record<UserRole, { label: string; class: string }> = {
  ...
  AUDITOR: { label: 'Auditor', class: 'bg-amber-100 text-amber-700 border-amber-200' },
};
...
export function AppShell() {
  ...
  useEffect(() => {
    ...
    const sseUrl = `${apiBase.replace(/\/api\/?$/, '')}/api/integrity-stream${token ? `?token=${encodeURIComponent(token)}` : ''}`;
